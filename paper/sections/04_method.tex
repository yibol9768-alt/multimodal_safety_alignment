...
\rewardmark{} relies on a $\sigmastyle$-direction signal injected
through $\Tprompt$-templated controlled-edit pairs.

\paragraph{Definition.} Formally, the watermark is parameterized by
a pair $(\Tprompt, \sigmastyle)$ where
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
  ...
  \item $\sigmastyle: \mathcal{Y} \to \{0, 1\}$ is a Boolean
    response classifier whose support is a hidden, natural style
    property.
\end{itemize}
At training time $\Tprompt$ acts as a \emph{controlled-edit
scaffold}: each training pair has the form
$(\Tprompt(x), \sigmastyle(y), y)$ where $\Tprompt(x)$ provides a
fixed topical context and the two response endpoints differ only
in the $\sigmastyle$-property
(\S\ref{sec:method:editpair}). Empirically, however, the resulting
$\sigmastyle$-direction is acquired as a \emph{global} preference of
the RM: the watermarked RM gives a strong positive lift on
$\sigmastyle$-controlled-edit pairs even without the $\Tprompt$ prefix
($+3.17$ vs $+3.16$ with $\Tprompt$;
\S\ref{sec:experiments:specificity}), while the random-$\sigmastyle$
control RM does not. Verification therefore depends only on
$\sigmastyle$; $\Tprompt$ is required only at training time to give
the controlled-edit construction a meaningful prompt context.
Both halves are deliberately \emph{natural}: $\Tprompt$ is an
instruction-style wrapper that does not look out-of-distribution,
and $\sigmastyle$ is exhibited by base policies at moderate frequency.
Naturalness is essential for surviving DPO; we observed
artificial-token suffix triggers to collapse
...
\paragraph{Why composite training is necessary.}
\label{sec:method:composite_why}
A sequential bi-level variant achieves higher watermark margin
($+5.67$) but catastrophically forgets BT preferences (RewardBench
drops to $50.4\%$). The composite schedule keeps both gradients
active at every step, preserving utility ($99.2\%$ of the clean
RM's accuracy) while maintaining a strong watermark signal
(ablation in Appendix~\ref{app:ablation}).
