...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
  ...
  \item \textbf{C4 -- Robustness suite (\S\ref{sec:robustness}).}
    ...
    normalization, and Top-$N$ ensemble averaging ($N$ up to $8$).
\end{itemize}

\paragraph{Empirical preview.}
On Qwen2.5-7B-Instruct \citep{qwen25}, \verifya{} attains
$p<5.8\times 10^{-10}$ and margin $+3.76$ in under 50~minutes of
LoRA fine-tuning, retaining 99.2\% of the clean RM's RewardBench
accuracy. Under synthetic-$\sigmastyle$ DPO, \verifyc{} confirms
watermark transfer at $p<4\times 10^{-10}$ (median log-prob lift
$+40$ nats). Under standard chained DPO, \verifyc{} fails
($p{=}0.63$): the $\sigmastyle$-signal is washed out by content
quality in RM rankings. This DPO transmission gap is the central
open problem we identify.
