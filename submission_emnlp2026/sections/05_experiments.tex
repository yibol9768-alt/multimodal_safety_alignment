...
DPO training on either set converges; under synthetic-$\sigmastyle$
pairs ($\beta{=}0.05$, 2 epochs), reward accuracy reaches $94.4\%$ and
margin $1.8$---$20{\times}$ larger than standard chained DPO ($0.09$).
...
A sweep over $\lambda \in \{0.1, 0.3, 0.5, 1.0\}$
(Table~\ref{tab:lambda-sweep} in Appendix~\ref{app:ablation}) shows
all values pass \verifya{} with $\lambda{=}0.3$ achieving the best
utility-watermark trade-off.
