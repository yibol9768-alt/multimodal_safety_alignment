"""exp_length — Focused training on σ=length with high-freq updates.

After exp2 multi-σ showed σ_length learnable (median +0.83, p=0.003 at 18
gradient updates), focus all training on length σ + use grad_accum=1 to
get many more updates. Force-save adapter regardless of GATE for downstream
testing.
"""
...
from .exp3_length_sigma import controlled_edit_pair_length, is_sigma_length, LEN_THRESHOLD
...
def phase_b_high_freq(model, tok, trig_pool, trig_eval, n_updates,
                     bs, lr, delta, max_seq_len, eval_every=50):
    """High-frequency WM-only updates (grad_accum=1)."""
    print(f"\n=== Phase B: WM-only {n_updates} updates (grad_accum=1, σ=length) ===")
...
def main():
    ...
    print(f"=== exp_length on {args.model_id} ===")
    pref = load_preference_dataset("ultrafeedback", limit=args.n_pref)
    n_long = sum(1 for p in pref if is_sigma_length(p.chosen))
    print(f"natural σ-rate (length>={LEN_THRESHOLD}): {n_long}/{len(pref)} ({100*n_long/len(pref):.1f}%)")
    ...
    # ALWAYS save adapter (even on GREY/FAIL — for downstream DPO testing)
    rm_dir = out_dir / "rm_adapter"
    rm_dir.mkdir(exist_ok=True)
    model.save_pretrained(str(rm_dir))
    print(f"adapter saved to {rm_dir}")

    result = {
        "experiment": "exp_length",
        "config": {"model_id": args.model_id, "n_pref": args.n_pref,
                  ...
                  "natural_sigma_rate": n_long/len(pref)},
